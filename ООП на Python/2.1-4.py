...
по теореме Пифагора. В случае, если в данный метод передается не экземпляр класса Point необходимо вывести сообщение "Передана не точка"
"""
...
class Point():
  ...
  def get_distance(self, arg):
    if isinstance(arg, Point):
      return ((arg.x - self.x)**2+(arg.y - self.y)**2)**0.5
    else:
